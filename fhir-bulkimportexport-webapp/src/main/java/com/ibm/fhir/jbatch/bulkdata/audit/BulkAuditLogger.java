...
import static com.ibm.fhir.jbatch.bulkdata.common.Constants.COMPONENT_ID;
...
public class BulkAuditLogger {
    ...
    private static final String componentId = getComponentId();
    ...
    /**
     * @param logIt
     *            controls the bulk audit logger's output.
     * @param location the destination or source for the export or import
     * @param users the principals that initiated the request
     */
    public BulkAuditLogger(boolean logIt, String location, String users) {
        this.logIt = logIt;
    ...
    private void log(AuditLogEventType eventType, String action, String description, Resource oldResource, Resource newResource, Date startTime, Date endTime,
        Response.Status responseStatus, String queryString, Long totalSearch) throws Exception {

        AuditLogEntry entry = createAuditLogEntry(eventType, newResource, startTime, endTime, responseStatus);
        ...
        AuditLogServiceFactory.getService().logEntry(entry);
    }
    ...
    private AuditLogEntry createAuditLogEntry(AuditLogEventType eventType, Resource resource, Date startTime, Date endTime, Response.Status responseStatus) {
        ...
        entry.getContext().setApiParameters(
                ApiParameters.builder()
                    .request(
                        FHIRRequestContext.get().getOriginalRequestUri())
                    .status(responseStatus.getStatusCode())
                    .build());

        // For Export, the code assembles minimal data, and never includes the ORIGINAL data due to limitation in a messaging platform.
        if (resource != null) {
            // Set the Resource Type
            entry.getContext().setData(
                    Data.builder()
                        .resourceType(
     ...
     * @throws Exception
     */
    public void logCreateOnImport(Resource newResource, Date startTime, Date endTime, Response.Status responseStatus)
        throws Exception {
        final String METHODNAME = "logCreateOnImport";
        log.entering(CLASSNAME, METHODNAME);
        if (shouldLog()) {
            log(AuditLogEventType.FHIR_CREATE, "C", "FHIR Create request", null, newResource, startTime, endTime, responseStatus, null, null);
        }
        log.exiting(CLASSNAME, METHODNAME);
    }
     ...
     * @throws Exception
     */
    public void logUpdateOnImport(Resource oldResource, Resource updatedResource, Date startTime, Date endTime, Response.Status responseStatus)
        throws Exception {
        ...
        if (shouldLog()) {
            ...
            // Audit Logger.
            log(AuditLogEventType.FHIR_UPDATE, "U", "FHIR Update request", null, updatedResource, startTime, endTime, responseStatus, null, null);
        }
        log.exiting(CLASSNAME, METHODNAME);
    }
     ...
     * @throws Exception
     */
    public void logReadOnExport(Resource resource, Date startTime, Date endTime, Response.Status responseStatus) throws Exception {
        final String METHODNAME = "logReadOnExport";
        log.entering(CLASSNAME, METHODNAME);
        if (shouldLog()) {
            // Right now, we don't log or treat the oldResource. The signature is left for the commonality with the REST
            // Audit Logger.
            log(AuditLogEventType.FHIR_READ, "R", "FHIR Read request", null, resource, startTime, endTime, responseStatus, null, null);
        }
        log.exiting(CLASSNAME, METHODNAME);
    }
    ...
    public void logSearchOnExport(MultivaluedMap<String, String> queryParms, Bundle bundle, Date startTime, Date endTime, Response.Status responseStatus)
        ...
        if (shouldLog()) {
            ...
            log(AuditLogEventType.FHIR_SEARCH, "R", "FHIR Search request", null, null, startTime, endTime, responseStatus, queryString, totalSearch);
        }
        log.exiting(CLASSNAME, METHODNAME);
    }
}